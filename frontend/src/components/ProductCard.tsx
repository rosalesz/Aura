...
const ProductCard: React.FC<ProductCardProps> = ({ product }) => {
  return (
    <Link to={`/products/${product.id}`} className="group relative bg-slate-900 border border-slate-800 hover:border-cyan-500/50 transition-colors duration-300 block">
      {/* Image Container */}
      <div className="relative aspect-[4/5] overflow-hidden bg-slate-800">
        ...
        />
        {/* Quick Add Overlay */}
        <div className="absolute bottom-0 right-0 p-4 bg-white text-black translate-y-full group-hover:translate-y-0 transition-transform duration-300 flex items-center gap-2 font-mono text-xs font-bold">
